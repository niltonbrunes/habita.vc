      ...
      >
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-3">
            <div 
              className="w-12 h-12 rounded-2xl flex items-center justify-center text-[11px] font-black text-white shadow-lg border-2 border-white"
        ...
        {(lead.property || lead.interest_description) && (
          <div className="mb-6 p-4 bg-muted/30 rounded-2xl border border-border/20 group/prop hover:bg-white hover:border-accent/20 transition-all">
            <div className="flex items-start gap-3">
              <div className="mt-0.5 p-1.5 bg-white rounded-lg shadow-sm">
                {lead.property ? <Building size={12} className="text-primary" /> : <MapPin size={12} className="text-accent" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[8px] font-black text-muted-foreground/60 uppercase tracking-widest mb-0.5">
                  {lead.property ? 'Imóvel da Base' : 'Interesse de Mercado'}
                </p>
        ...
        <div className="space-y-4 mb-6">
          <div className="flex items-end justify-between">
            <div className="space-y-1">
              <span className="text-[8px] font-black text-muted-foreground/60 uppercase tracking-widest">Conversão</span>
              <p className="text-lg font-black text-primary leading-none">{lead.probability || 0}%</p>
            </div>
            <div className="text-right space-y-1">
              <span className="text-[8px] font-black text-muted-foreground/60 uppercase tracking-widest">Valor do Negócio</span>
              <p className="text-sm font-black text-primary leading-none">{formattedValue}</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border/30">
          ...
          <div className={`w-2 h-2 rounded-full ${
            lead.temperature === 'hot' ? 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.4)]' : 
            lead.temperature === 'warm' ? 'bg-blue-500' : 'bg-slate-300'
          }`} />
        </div>
      </div>
    </motion.div>
